...
if (!empty($url_generar)) {
    ...
    if (empty($url_id)) {
        ...
        $porcentaje_mano_obra_consorciado  = SQL::obtenerValor("registro_obras", "porcentaje_mano_obra", "id = '$url_id'");
        $porcentaje_materiales_consorciado = SQL::obtenerValor("registro_obras", "porcentaje_materiales", "id = '$url_id'");
        $valor_factura_cliente             = SQL::obtenerValor("registro_obras", "valor_factura_cliente", "id = '$url_id'");
    ...
    }if($datosRegistroObra->pago_consorciado == 1){
        ...
        /*** Definición de pestañas ***/
            ...
            $formularios["PESTANA_DATOS_ACTA"] = array(
                ...
                array(
                    HTML::campoTextoCorto("numero_factura_consorciado", $textos["NUMERO_FACTURA_CONSORCIADO"], 12, 15, "", array("title" => $textos["AYUDA_NUMERO_FACTURA_CONSORCIADO"]))
                ),            
                array(
  	                HTML::campoOculto("id_obra", $url_id),
  	                ...
  	                HTML::campoOculto("valor_factura_cliente", $valor_factura_cliente),
  	                ...
  	                HTML::campoOculto("porcentaje_mano_obra_consorciado", $porcentaje_mano_obra_consorciado),
  	                HTML::campoOculto("porcentaje_materiales_consorciado", $porcentaje_materiales_consorciado),
    ...
    if(($forma_tipo_acta != '1') && ($forma_valor_facturar != $forma_total_general)){
        ...
        $valor_mano_obra_consorciado     = (($valor_mano_obra_cliente * $forma_porcentaje_mano_obra_consorciado) / 100);
        $valor_materiales_consorciado    = (($valor_materiales_cliente * $forma_porcentaje_materiales_consorciado) / 100);
        ...
        $valor_mano_obra_consorciado  = (($forma_valor_mano_obra_cotizacion * $forma_porcentaje_mano_obra) / 100);
        $valor_materiales_consorciado = (($forma_valor_materiales_cotizacion * $forma_porcentaje_materiales) / 100);
    }
    ...
    $consulta = SQL::modificar("registro_obras", $datos, "id = '$forma_id_obra'");
   
    if ($consulta) {
        $nombreArchivo      = $rutasGlobales["archivos"]."/pago_consorciado".$forma_id_acta.".pdf";
        $nombreArchivo2     = "pago_consorciado".$forma_id_acta.".pdf";
        ...
        $archivo->Cell(21,8,"".$textos["INFORME_CONSORCIADO"]."",0);
        ...
        $valor_mano_obra_consorciado = quitarMiles($valor_mano_obra_consorciado);
        $archivo->SetFont('Arial','',8);
        $archivo->Cell(60,8,"".$textos["SIMBOLO_PESO"].number_format($valor_mano_obra_consorciado)."",0);
        $archivo->Cell(20,8,"",0,1,'R');

        $archivo->SetFont('Arial','B',8);
        $archivo->Cell(30,8,$textos["VALOR_MATERIALES_COTIZACION"]." :",0,0,'L');
            
        $valor_materiales_consorciado = quitarMiles($valor_materiales_consorciado);
        $archivo->SetFont('Arial','',8);
        $archivo->Cell(60,8,"".$textos["SIMBOLO_PESO"].number_format($valor_materiales_consorciado)."",0);
        $archivo->Cell(20,8,"",0,1,'R');
