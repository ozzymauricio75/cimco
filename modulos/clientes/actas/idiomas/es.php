<?php

/**
*
* Copyright (C) 2009 FELINUX Ltda
* Francisco J. Lozano B. <fjlozano@felinux.com.co>
* Mauricio Oidor L. <ozzymauricio75@gmail.com>
*
* Este archivo es parte de:
* PANCE :: Plataforma para la Administración del Nexo Cliente-Empresa
*
* Este programa es software libre: usted puede redistribuirlo y/o
* modificarlo  bajo los términos de la Licencia Pública General GNU
* publicada por la Fundación para el Software Libre, ya sea la versión 3
* de la Licencia, o (a su elección) cualquier versión posterior.
*
* Este programa se distribuye con la esperanza de que sea útil, pero
* SIN GARANTÍA ALGUNA; ni siquiera la garantía implícita MERCANTIL o
* de APTITUD PARA UN PROPÓSITO DETERMINADO. Consulte los detalles de
* la Licencia Pública General GNU para obtener una información más
* detallada.
*
* Debería haber recibido una copia de la Licencia Pública General GNU
* junto a este programa. En caso contrario, consulte:
* <http://www.gnu.org/licenses/>.
*
**/

$textos = array(
    "GESTACTA"                             => "Mantenimiento actas",                                   
    "MODIACTA"                             => "Modificar acta",                 
    "CONSACTA"                             => "Consultar acta",                 
    "ELIMACTA"                             => "Eliminar acta",   
    "NOTIACTA"                             => "Enviar factura cliente",
    "REPOACTA"                             => "Reporte actas",
    "PAGOCLIE"                             => "Confirmar pago cliente",
    "PAGOCONS"                             => "Confirmar pago consorciado",
    "SEDE"                                 => "Sede",                             
    "CARGO"                                => "Cargo", 
    "IMAGEN"                               => "Imagen",
    "INFORME"                              => "Informe",
    "FACTURA"                              => "Factura",
    "GENERAR_PLANO"                        => "Generar plano",
    "NUMERO_FACTURA"                       => "Factura cimco",
    "NUMERO_FACTURA_CONSORCIADO"           => "Factura consorciado",
    "NOTIFICAR"                            => "Enviar factura cliente",
    "NOTIFICAR_CLIENTE"                    => "Confirmar pago",
    "NOTIFICAR_CONSORCIADO"                => "Confirmar pago",
    "CONFIRMAR_CLIENTE"                    => "Pago cliente",
    "CONFIRMAR_CONSORCIADO"                => "Pago consorciado",
    "ASUNTO"                               => "Asunto",
    "TEMA_ASUNTO"                          => "Acta cotización",
    "INFORME_CONSORCIADO"                  => "Informe consorciado",
    "INFORME_CLIENTE"                      => "Informe cliente",
    "ATTE"                                 => "Señor(a)",
    "SALUDO"                               => "Cordial saludo",
    "TITULO"                               => "Nos permitimos entregarle la información del acta referente a la cotización para su respectiva factura.",
    "IMPRIMIR_PDF"                         => "Descargar PDF",
    "CORDIALMENTE"                         => "Cordialmente,",
    "TIPO_SOLICITUD"                       => "Tipo solicitud",   
    "TIPO_ACTA"                            => "Tipo acta",
    "FECHA_DESDE"                          => "Fecha desde",
    "FECHA_HASTA"                          => "Fecha hasta",
    "FECHA_INGRESO"                        => "Fecha ingreso",    
    "FECHA_ENTREGA_ACTA"                   => "Fecha entrega",
    "DESCRIPCION"                          => "Descripción ", 
    "CONTACTO"                             => "Contacto",
    "DESCRIPCION"                          => "Descripción ",                    
    "NOTIFICADO"                           => "Notificado",            
    "ESTADO_COTIZACION"                    => "Estado cotización",     
    "ESTADO"                               => "Estado",
    "MUNICIPIO"                            => "Ubicación sede",        
    "SUCURSAL"                             => "Consorciado",           
    "TODAS"                                => "Todas las actas",
    "ACTA_INICIO"                          => "Acta inicio", 
    "ACTA_AVANCE_OBRA"                     => "Acta avance obra",
    "ACTA_FINALIZACION"                    => "Acta finalización",
    "ESTADO_REQUERIMIENTO"                 => "Estado requerimiento",
    "FACTURA_CLIENTE"                      => "Factura cliente",
    "PAGO_CLIENTE"                         => "Pago cliente",
    "PAGO_CONSORCIADO"                     => "Pago consorciado",
    "PESTANA_CONSORCIADO"                  => "Consorciados",                                       
    "PESTANA_COTIZACION"                   => "Datos cotización",                                       
    "PESTANA_REQUERIMIENTO"                => "Información requerimiento",                                 
    "PESTANA_REPORTE"                      => "Archivo plano",
    "PESTANA_DATOS_CONTROL"                => "Datos control",
    "PESTANA_ACTAS"                        => "Actas",
    "PESTANA_IMAGEN"                       => "Imagen",
    "PESTANA_INFORMES"                     => "Informes",
    "PESTANA_DATOS_ACTA"                   => "Datos actas",
    "OBSERVACIONES"                        => "Observaciones",
    "OBSERVACIONES"                        => "Observaciones generadas por el cliente",                    
    "OBSERVACIONES_VISITA"                 => "Observaciones visita",    
    "OBSERVACIONES_APROBACION"             => "Observaciones aprobación",
    "NUMERO_COTIZACION"                    => "Numero cotización",  
    "NUMERO_COTIZACION_CONSORCIADO"        => "Numero cotización consorciado",
    "VALOR_REQUERIMIENTO"                  => "Valor requerimiento",                      
    "VALOR_MANO_OBRA_COTIZACION"           => "Valor mano de obra",                                              
    "VALOR_MANO_OBRA_MODIFICADA"           => "Valor mano de obra propuesto",                                    
    "VALOR_MATERIALES_COTIZACION"          => "Valor materiales",                                                
    "VALOR_MATERIALES_MODIFICADO"          => "Valor materiales propuesto",               
    "VALOR_ADMINISTRACION_COTIZACION"      => "Valor administración",                     
    "VALOR_IMPREVISTOS_COTIZACION"         => "Valor imprevistos",                        
    "VALOR_UTILIDAD"                       => "Valor utilidad",                           
    "VALOR_IMPUESTO"                       => "Valor impuesto",                           
    "VALOR_ANTICIPO"                       => "Valor anticipo", 
    "VALOR_FACTURAR"                       => "Valor acta",
    "VALOR_PAGO_CLIENTE"                   => "Valor pago cliente",
    "VALOR_PAGO_CONSORCIADO"               => "Valor pago consorciado",
    "VALOR_ADMINISTRACION"                 => "Valor pago administración",
    "VALOR_CANCELADO"                      => "Valor cancelado",
    "VALOR_FACTURAR_MODIFICADO"            => "Valor facturado",
    "FALTA_FACTURAR"                       => "Para esta cotización solo puede facturar: $",
    "SUB_TOTAL"                            => "Sub-Total",                                
    "TOTAL_GENERAL"                        => "Total general", 
    "ACUMULADO"                            => "Acumulado actas",
    "PORCENTAJE_ADMINISTRACION_COTIZACION" => "Porcentaje admin",                          
    "PORCENTAJE_ADMINISTRACION_MODIFICADO" => "Porcentaje admin propuesto",               
    "PORCENTAJE_IMPREVISTOS_COTIZACION"    => "Porcentaje imprev",                        
    "PORCENTAJE_IMPREVISTOS_MODIFICADO"    => "Porcentaje imprev propuesto",              
    "COSTO_DIRECTO"                        => "Costo directo",                            
    "COSTO_DIRECTO_MODIFICADO"             => "Costo directo propuesto",                  
    "PORCENTAJE_UTILIDAD"                  => "Porcentaje utilidad",                      
    "PORCENTAJE_UTILIDAD_MODIFICADA"       => "Porcentaje utilidad propuesto",            
    "IMPUESTO"                             => "Impuesto",                                 
    "IMPUESTO_MODIFICADO"                  => "Impuesto propuesto",                       
    "FORMA_PAGO"                           => "Forma de pago",                                                   
    "FORMA_PAGO_MODIFICADO"                => "Forma de pago propuesta",                                         
    "PORCENTAJE_ANTICIPO"                  => "Porcentaje anticipo",                                             
    "PORCENTAJE_ANTICIPO_MODIFICADO"       => "Porcentaje anticipo propuesto",                                   
    "PORCENTAJE_MANO_OBRA"                 => "Porcentaje mano obra",                                            
    "PORCENTAJE_MANO_OBRA_MODIFICADO"      => "Porcentaje mano obra propuesto",                                  
    "PORCENTAJE_MATERIALES"                => "Porcentaje materiales",                                           
    "PORCENTAJE_MATERIALES_MODIFICADO"     => "Porcentaje materiales propuesto",
    "NUMEROCOTIZACION"                     => "Numero cotización",
    "TIPOACTA"                             => "Tipo acta",
    "FECHAENTREGA"                         => "Fecha entrega",
    "VALORFACTURAR"                        => "Valor facturado",
    "FACTURACONSORCIADO"                   => "Factura consorciado",
    "PAGOCLIENTE"                          => "Pago cliente",
    "PAGOCONSORCIADO"                      => "Pago consorciado",
    "AYUDA_INFORME"                        => "Informe especial sobre el acta",
    "AYUDA_NUMERO_FACTURA"                 => "Numero de factura del consorcio",
    "AYUDA_NUMERO_FACTURA_CONSORCIADO"     => "Numero de factura del consorciado",
    "AYUDA_IMAGEN"                         => "Imagen del acta",
    "AYUDA_TIPO_ACTA"                      => "Tipo del acta",
    "AYUDA_FECHA_ENTREGA_ACTA"             => "Fecha de entrega del acta",
    "AYUDA_FECHA_DESDE"                    => "Fecha inicio del reporte",
    "AYUDA_FECHA_HASTA"                    => "Fecha final del reporte",
    "AYUDA_VALOR_FACTURAR"                 => "Valor facturación del acta",
    "AYUDA_VALOR_CANCELADO"                => "Valor cancelado por el cliente",
    "AYUDA_PORCENTAJE_MANO_OBRA"           => "Porcentaje mano de obra",
    "AYUDA_PORCENTAJE_MATERIALES"          => "porcentaje materiales",
    "ERROR_EXISTE_ACTA"                    => "Ya existe un acta con ese numero",
    "ERROR_ACTA_ENVIADA"                   => "Está operación no es posible, porque la factura ya fue enviada",
    "ERROR_ACTA_PAGADA"                    => "Está operación no es posible, porque el pago ya fue efectuado",
    "ERROR_COMPLETO_OBRA"                  => "El acumulado de las actas ya excedió el valor total de la cotización",
    "ERROR_FORMATO_VALOR_FACTURAR"         => "El campo valor facturar debe contener solo números",
    "ERROR_FORMATO_PORCENTAJE_MANO_OBRA"   => "El campo porcentaje de la mano de obra debe contener solo números",
    "ERROR_FORMATO_PORCENTAJE_MATERIALES"  => "El campo porcentaje materiales debe contener solo números",
    "TELEFONO_CONTACTO"                    => "Telefono del contacto",
    "CODIGO_CONTABLE"                      => "Codigo contable",
    "PERSONA_RECIBE"                       => "Persona recibio requerimiento",
    "MEDIO_RECIBO"                         => "Medio de recibo del requerimiento",
    "NUMERO_REQUERIMIENTO"                 => "Numero requerimiento",
    "BASE_IMPUESTO"                        => "Base para impuesto",
    "MANTENIMIENTO"                        => "Mantenimiento",
    "EMERGENCIA"                           => "Emergencia",
    "SERVICIO"                             => "Servicio por demanda",
    "PROYECTO"                             => "Proyecto",
    "VISITA"                               => "Visita",
    "SELECCIONAR_TODOS"                    => "Seleccionar todos",
    "CONSORCIADOS"                         => "Consorciados",
    "ERROR_FECHAS"                         => "Fecha hasta menor que fecha desde",
    "PENDIENTE"                            => "Pendiente",
    "EJECUCION"                            => "En ejecución",
    "DESCARTADA"                           => "Descartada",
    "FACTURADA_TOTAL"                      => "Facturada total", 
    "FACTURADA_PARCIAL"                    => "Facturada parcial",
    "EJECUTADA"                            => "Ejecutada",
    "REEMPLAZADA"                          => "Reemplazada",
    "RECOTIZADA"                           => "Recotizada",
    "ABRIR_PLANO"                          => "Descargar archivo",
    "NUMERO_FACTURA"                       => "Numero factura cliente",
    "NUMERO_FACTURA_CONSORCIADO"           => "Numero factura consorciado",
    "AYUDA_NUMERO_FACTURA"                 => "Numero factura cliente",
    "AYUDA_NUMERO_FACTURA_CONSORCIADO"     => "Numero de factura del consorciado"
);                                                                                                             
?>
