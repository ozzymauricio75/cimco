...
if (!empty($url_generar)) {
    ...
    $formularios["PESTANA_GENERAL"] = array(
        ...
        array(
            HTML::campoTextoCorto("codigo_municipal", $textos["CODIGO_MUNICIPAL"], 3, 3, "", array("title" => $textos["AYUDA_CODIGO_MUNICIPAL"])),
            HTML::campoTextoCorto("codigo_interno", $textos["CODIGO_INTERNO"], 4, 4, "", array("title" => $textos["AYUDA_CODIGO_INTERNO"])),
        ),
        array(
            HTML::campoTextoCorto("estrato", $textos["ESTRATO"], 2, 2, "", array("title" => $textos["AYUDA_ESTRATO"])),
            HTML::campoTextoCorto("comuna", $textos["COMUNA"], 2, 2, "", array("title" => $textos["AYUDA_COMUNA"]))
        )
    );
...
/*** Validación en línea de los datos provenientes del formulario ***/
    ...
    if ($url_item == "codigo_municipal" && $url_valor) {
        $existe = SQL::existeItem("localidades", "codigo_municipal", $url_valor);

        if ($existe) {
            $respuesta =  $textos["ERROR_EXISTE_CODIGO_MUNICIPAL"];

        } elseif (!Cadena::validarNumeros($url_valor, 3, 3)) {
            $respuesta =  $textos["ERROR_FORMATO_CODIGO_MUNICIPAL"];
        }
    }
...
/*** Adicionar los datos provenientes del formulario ***/
    ...
    if (empty($forma_municipio) || empty($forma_nombre)) {
    ...
    } elseif (!empty($forma_codigo_municipal) && SQL::existeItem("localidades", "codigo_municipal", $forma_codigo_municipal, "id_municipio = '$forma_municipio'")) {
            $error   = true;
            $mensaje =  $textos["ERROR_EXISTE_CODIGO_MUNICIPAL"];

    } elseif (!empty($forma_codigo_municipal) && !Cadena::validarNumeros($forma_codigo_municipal, 3, 3)) {
        $error   = true;
        $mensaje =  $textos["ERROR_FORMATO_CODIGO_MUNICIPAL"];
        ...
        $datos = array(
            ...
            "nombre"           => $forma_nombre,
            "codigo_municipal" => $forma_codigo_municipal,
