...
if (!empty($url_generar)) {
    ...
    if (empty($url_id)) {
        ...
        $pais          = SQL::obtenerValor("departamentos", "id_pais", "id = '".$datos->id_departamento."'");
        ...
        $formularios["PESTANA_GENERAL"] = array(
            ...
            array(
                HTML::listaSeleccionSimple("*departamento", $textos["DEPARTAMENTO"], $departamentos, $datos->id_departamento, array("title" => $textos["AYUDA_DEPARTAMENTO"]))
            ),
            array(
                HTML::campoTextoCorto("*nombre", $textos["NOMBRE"], 30, 255, $datos->nombre, array("title" => $textos["AYUDA_NOMBRE"]))
            ),
            array(
                HTML::campoTextoCorto("codigo_dane", $textos["CODIGO_DANE"], 3, 3, $datos->codigo_dane, array("title" => $textos["AYUDA_CODIGO_DANE"])),
            ...
            array(
                HTML::marcaChequeo("capital", $textos["CAPITAL"], 1, $datos->capital)
            )
        );
...
/*** Modificar el elemento seleccionado ***/
    ...
    if (empty($forma_departamento) || empty($forma_nombre)) {
    ...
    } elseif (SQL::existeItem("municipios", "nombre", $forma_nombre, "id != '$forma_id' AND id_departamento = '$forma_departamento'")) {
        $error   = true;
        $mensaje =  $textos["ERROR_EXISTE_NOMBRE"];

    } elseif (!empty($forma_codigo_dane) && SQL::existeItem("municipios", "codigo_dane", $forma_codigo_dane, "id != '$forma_id' AND id_departamento = '$forma_departamento'")) {
            $error   = true;
    ...
    } elseif (!empty($forma_codigo_interno) && SQL::existeItem("municipios", "codigo_interno", $forma_codigo_interno, "id != '$forma_id' AND id_departamento = '$forma_departamento'")) {
        $error   = true;
        ...
        $consulta = SQL::modificar("municipios", $datos, "id = '$forma_id'");
