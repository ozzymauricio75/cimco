...
//                HTML::mostrarDato("base_utilidad", $base_utilidad, $simbolo_util_peso.$valor_base_utilidad),
        ...
        $consulta_actas = SQL::seleccionar(array("registro_obras"),array("*"),"id_cotizacion='$url_id'");
        if(SQL::filasDevueltas($consulta_actas)){
            $actas = array();
            while($datos_actas=SQL::filaEnObjeto($consulta_actas)){            
                if(($datos_actas->valor_pago_cliente)>0){
                    
                }        
            }
        }
        
	    $vistaConsulta	= "registro_obras";
	    $alineacion     = array("I","D","D","I");	    $columnas	    = array("id","ARTICULO","CANTIDAD","VALOR_VENTA","MARGEN");	    $condicion	    = "id_cliente_mayorista = '$url_id_cliente_mayorista' AND id_sucursal = '$url_id_sucursal'";	    $consulta	    = SQL::seleccionar(array($vistaConsulta), $columnas, $condicion);
        $tabla          = HTML::generarTabla($columnas, $consulta, $alineacion, "tablaPendientes",false);
    ...
    if($forma_estado != '2'){
        ...
        if ($consulta) {
            ...
            if (
                (($forma_costo_utilidad) || $forma_costo_utilidad != 0)
               ){
