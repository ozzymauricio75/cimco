...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`cimco`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `pance_buscador_registro_obras` AS select `pance_cotizaciones`.`id` AS `id`,`pance_requerimientos_clientes`.`fecha_ingreso` AS `fecha_ingreso`,`pance_requerimientos_clientes`.`id_sede` AS `id_sede`,`pance_sedes_clientes`.`nombre_sede` AS `nombre_sede`,`pance_sedes_clientes`.`id_municipios` AS `municipio`,`pance_requerimientos_clientes`.`id_sucursal` AS `sucursal`,`pance_registro_obras`.`valor_facturar` AS `valor_facturar`,(case `pance_requerimientos_clientes`.`tipo_solicitud` when _latin1'M' then _latin1'Mantenimiento' when _latin1'E' then _latin1'Emergencia' when _latin1'S' then _latin1'Servicio por demanda' when _latin1'P' then _latin1'Proyecto' end) AS `tipo_solicitud`,`pance_requerimientos_clientes`.`descripcion` AS `descripcion` from (((((`pance_requerimientos_clientes` join `pance_sedes_clientes`) join `pance_sucursales`) join `pance_cotizaciones`) join `pance_municipios`) join `pance_registro_obras`) where ((`pance_cotizaciones`.`id_requerimiento` = `pance_requerimientos_clientes`.`id`) and (`pance_requerimientos_clientes`.`id_sede` = `pance_sedes_clientes`.`id`) and (`pance_requerimientos_clientes`.`id_sucursal` = `pance_sucursales`.`id`) and (`pance_municipios`.`id` = `pance_sedes_clientes`.`id_municipios`)) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`cimco`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `pance_menu_registro_obras` AS select `pance_cotizaciones`.`id` AS `id`,`pance_sedes_clientes`.`nombre_sede` AS `SEDE`,`pance_sucursales`.`nombre` AS `SUCURSAL`,`pance_municipios`.`nombre` AS `MUNICIPIO`,`pance_requerimientos_clientes`.`descripcion` AS `DESCRIPCION`,(case `pance_requerimientos_clientes`.`tipo_solicitud` when _latin1'M' then _latin1'Mantenimiento' when _latin1'E' then _latin1'Emergencia' when _latin1'S' then _latin1'Servicio por demanda' when _latin1'P' then _latin1'Proyecto' end) AS `TIPO_SOLICITUD`,`pance_requerimientos_clientes`.`fecha_ingreso` AS `FECHA_INGRESO` from ((((`pance_requerimientos_clientes` join `pance_sedes_clientes`) join `pance_sucursales`) join `pance_municipios`) join `pance_cotizaciones`) where ((`pance_requerimientos_clientes`.`id_sede` = `pance_sedes_clientes`.`id`) and (`pance_requerimientos_clientes`.`id_sucursal` = `pance_sucursales`.`id`) and (`pance_sedes_clientes`.`id_municipios` = `pance_municipios`.`id`) and (`pance_requerimientos_clientes`.`estado_aprobacion_requerimiento` = _latin1'1') and (`pance_requerimientos_clientes`.`id` = `pance_cotizaciones`.`id_requerimiento`) and (`pance_cotizaciones`.`estado` = _latin1'2')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`cimco`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `pance_menu_terceros` AS select `pance_terceros`.`id` AS `id`,`pance_terceros`.`documento_identidad` AS `DOCUMENTO_IDENTIDAD`,`pance_terceros`.`primer_nombre` AS `PRIMER_NOMBRE`,`pance_terceros`.`segundo_nombre` AS `SEGUNDO_NOMBRE`,`pance_terceros`.`primer_apellido` AS `PRIMER_APELLIDO`,`pance_terceros`.`segundo_apellido` AS `SEGUNDO_APELLIDO`,concat(`pance_terceros`.`primer_nombre`,_latin1' ',`pance_terceros`.`segundo_nombre`,_latin1' ',`pance_terceros`.`primer_apellido`,_latin1' ',`pance_terceros`.`segundo_apellido`) AS `NOMBRE_COMPLETO` from `pance_terceros` */;
...
-- Dump completed on 2009-08-03 21:08:43
