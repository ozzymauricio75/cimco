...
class HTML {
    ...
    public static function generarPestanas($formularios, $botones = "", $opciones = "") {
        ...
        foreach ($formularios as $pestana => $filas) {
            ...
            $lista .= "<li class=\"ui-tabs-nav-item\"><a href=\"#$pestana\">".$textos[$pestana]."</a></li>\n";
    ...
    /*** Crear código para insertar un campo oculto ***/
    ...
    /*** Generar elemento de formulario para ingreso de texto de varias líneas ***/
    ...
    public static function selectorArchivo($id, $etiqueta, $opciones) {
        ...
        $elemento .= "<input type=\"file\" id=\"$id\" name=\"$id\" class=\"campo\" size=\"20\" $opciones>$requerido\n";

        return $elemento;
    }

    /*** Generar tabla HTML a partir de una vista SQL ***/
    public static function generarTabla($columnas, $filas, $alineacion, $id = "tablaPrincipal", $ordenarColumnas = true, $variable=null) {
	...
	$parImpar = "even";
		...
		/*** Obtener el id para la fila, si aplica ***/
                if ($datos[0]) {
                    $contenido .= " <tr id=\"fila_".$datos[0]."\" class=\"$parImpar\">";
                } else {
                    $contenido .= " <tr class=\"$parImpar\">";
                }
    ...
    private static function generarArbol($elemento = "") {
        ...
        $columnas     = array("id", "carpeta", "archivo");
        ...
        if ($privilegio == ""){
            ...
            if (SQL::filasDevueltas($resultado)) {
                ...
                while($datos = SQL::filaEnObjeto($resultado)) {
                    ...
                    $listaPrivilegios .= "<li class=\"dato\">\n<label for=\"privilegios[$item]\">\n";
                    $listaPrivilegios .= "<input type=\"checkbox\" id=\"$item\" class=\"campo\" name=\"privilegios[$item]\" $marcado value=\"1\" onchange=\"validarSeleccion(this)\">&nbsp;";
    ...
    private static function generarArbolGrupos($nodo, $padre, $marcador, $componente = "", $principal = false) {
        ...
        $columnas     = array("id", "codigo", "descripcion");
        $ordenamiento = "codigo ASC";

        if ($componente == "") {
            $condicion = "id_padre IS NULL";
            $resultado = SQL::seleccionar($tablas, $columnas, $condicion, $agrupamiento = "", $ordenamiento);
    ...
    private static function rutaGrupos($nodo) {
        ...
        $consulta = SQL::seleccionar(array("estructura_grupos"),array("id_padre"), "id = '$nodo'");
        $padre    = "";

        if (SQL::filasDevueltas($consulta)) {
            $datos = SQL::filaEnObjeto($consulta);
            $padre = $datos->id_padre;
        }
    ...
    private static function generarArbolContable($nodo, $padre, $marcador, $componente = "", $principal = false) {
        ...
        $columnas     = array("id", "codigo_contable", "descripcion");
        $ordenamiento = "codigo_contable ASC";

        if ($componente == "") {
            $condicion = "id_cuenta_padre IS NULL";
            $resultado = SQL::seleccionar($tablas, $columnas, $condicion, $agrupamiento = "", $ordenamiento);
            ...
            if (SQL::filasDevueltas($resultado)) {
                ...
                while ($datos = SQL::filaEnObjeto($resultado)) {
                    ...
                    $condicion    = "id_cuenta_padre = '$item'";
                    $resultado2   = SQL::seleccionar($tablas, $columnas, $condicion, $agrupamiento = "", $ordenamiento);
            ...
            $condicion = "id_cuenta_padre = '$componente'";
            $resultado = SQL::seleccionar($tablas, $columnas, $condicion, $agrupamiento = "", $ordenamiento);
    ...
    private static function rutaContable($nodo) {
        ...
        $consulta = SQL::seleccionar(array("plan_contable"),array("id_cuenta_padre"), "id = '$nodo'");

        if (SQL::filasDevueltas($consulta)) {
            $datos    = SQL::filaEnObjeto($consulta);
            $padre    = $datos->id_cuenta_padre;
